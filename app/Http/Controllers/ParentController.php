...
class ParentController extends Controller
{
    ...
    {
        $query = ParentModel::with('students')->withCount('students');
        ...
        $parents = $query->orderBy('last_name')->paginate(20)->withQueryString();
    ...
    public function notify(Request $request, ParentModel $parent)
    {
        $validated = $request->validate([
        ...
        \App\Models\ParentNotification::create([
            'parent_id' => $parent->id,
            'student_id' => $parent->students->first()->id ?? null, // Default to first student if any
            'subject' => $validated['subject'],
            'message' => $validated['message'],
            'notification_type' => 'email', // Default for now, user said "instead of email... do that soon", but DB needs enum
            'status' => 'pending',
            'sent_at' => now(), // Simulated immediate send
        ]);

        return back()->with('success', 'Notification sent to parent portal.');
    }
}
